...
var enterpriseCmd = &cobra.Command{
	Use:   "enterprise",
	Short: "Receive AWS Credentials by providing an enterprise name",
	Args:  cobra.ExactArgs(1),
	RunE: func(command *cobra.Command, args []string) error {
		ctx, cancel := signal.NotifyContext(context.Background(), syscall.SIGINT, syscall.SIGTERM)
		defer cancel()

		enterpriseName := args[0]
		...
		enterpriseID, err := ghClient.GetEnterpriseID(enterpriseName)
		if err != nil {
			return fmt.Errorf("failed to get enterprise ID: %w", err)
		}

		jwt, err := jwt.GetEnterpriseJWT(&jwt.JWTInputs{
			Audience: aud,
			GHToken:  ghToken,
			IDPUrl:   idpUrl,
		}, enterpriseID)
		if err != nil {
			return fmt.Errorf("failed to get enterprise JWT: %w", err)
		}
